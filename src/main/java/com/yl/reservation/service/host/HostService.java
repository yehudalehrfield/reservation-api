                        ...
                        return userRepository.findByUserId(requestHost.getUserId())
                                .flatMap(user -> {
                                    ...
                                    return hostRepository.save(requestHost)
                                            .map(createdHost -> new HostCreateUpdateResponse(
                                                    ResConstants.HOST_CREATE + createdHost.getHostId(), createdHost));
                                })
                                .switchIfEmpty(Mono.error(new ResGraphException(
                                        ResConstants.USER_NOT_FOUND_WITH_ID + requestHost.getUserId(),
                                        HttpStatus.BAD_REQUEST)));
        ...
        if (requestHost.getHostId() != null) {
            ...
            return hostRepository.findByHostId(hostIdtoSearch)
                    .flatMap(existingHost -> {
                        ...
                        return hostRepository.save(updateHost)
                                .map(host -> new HostCreateUpdateResponse(ResConstants.HOST_UPDATE + host.getHostId(),
                                        host));
                    })
                    .switchIfEmpty(Mono.error(new ResGraphException(
                            ResConstants.HOST_NOT_FOUND_WITH_ID + requestHost.getHostId(), HttpStatus.BAD_REQUEST)));
            ...
            return hostRepository.findByUserIdAndAddress(requestHost.getUserId(), requestHost.getAddress())
                    ...
                    .switchIfEmpty(
                            Mono.error(new ResGraphException(String.format(ResConstants.HOST_NOT_FOUND_USER_ID_ADDRESS,
                                    requestHost.getUserId(), requestHost.getAddress()), HttpStatus.BAD_REQUEST)));
        } else {
