...
public class HostService {
    ...
    public Mono<HostCreateUpdateResponse> createUpdateHost(HostCreateUpdateRequest hostCreateUpdateRequest) {
        ...
        if (hostCreateUpdateRequest.getHost() != null) {
            ...
            } else if (hostCreateUpdateRequest.getHost().getUserId() != null
                    && hostCreateUpdateRequest.getHost().getAddress() != null) {
                if (Boolean.TRUE.equals(hostCreateUpdateRequest.getIsAddressUpdate())) {
                    throw new ResGraphException(ResConstants.HOST_ID_REQUIRED_FOR_ADDRESS_UPDATE_ERROR,
        ...
        // no host is given. user must be given. update user.
            ...
            return updateUserInfo(hostCreateUpdateRequest, null, createUpdateDateTime)
                    .map(user -> new HostCreateUpdateResponse(
                            (user.getLastUpdated().equals(user.getCreatedDate()) ? ResConstants.USER_CREATE
                                    : ResConstants.USER_UPDATE) + user.getUserId(),
                            null, user));
    ...
    private Mono<HostCreateUpdateResponse> updateHostByHostId(HostCreateUpdateRequest hostCreateUpdateRequest,
        ...
        return hostRepository.findByHostId(hostCreateUpdateRequest.getHost().getHostId())
                .flatMap(hostToUpdate -> {
                    ...
                    if (Boolean.TRUE.equals(hostCreateUpdateRequest.getIsUserUpdate())) {
                        return zipUserUpdateWithHostUpdate(hostCreateUpdateRequest, hostToUpdate, updatedHost,
                                createUpdateDateTime);
                    }
                    return hostRepository.save(updatedHost)
                            .flatMap(host -> Mono.just(new HostCreateUpdateResponse(
                                    ResConstants.HOST_UPDATE + host.getHostId(), host, null)));
                })
                .onErrorResume(error -> Mono.error(new ResGraphException(error.getMessage(), HttpStatus.BAD_REQUEST)))
                .switchIfEmpty(Mono.error(new ResGraphException(
                        ResConstants.HOST_NOT_FOUND_WITH_ID + hostCreateUpdateRequest.getHost().getHostId(),
                        HttpStatus.BAD_REQUEST)));
    }

    private Mono<HostCreateUpdateResponse> updateHostByUserIdAndAddress(HostCreateUpdateRequest hostCreateUpdateRequest,
            String createUpdateDateTime) {
        return hostRepository.findByUserIdAndAddress(hostCreateUpdateRequest.getHost().getUserId(),
                ...
                .flatMap(hostToUpdate -> {
                    ...
                    return hostRepository.save(updatedHost)
                            .flatMap(savedHost -> Mono.just(new HostCreateUpdateResponse(
                                    ResConstants.HOST_UPDATE + savedHost.getId(), savedHost, null)));
                })
    ...
    // this will be handled with changes to the host api's - remove user update
    // entirely. why should we further complicate...
    private Mono<User> updateUserInfo(HostCreateUpdateRequest hostCreateUpdateRequest, String userId,
            String createUpdateDateTime) {
        ...
        if (hostCreateUpdateRequest.getUser().getUserId() != null || userId != null) {
            ...
            return userRepository.findByUserId(userIdToSearch)
                    .flatMap(user -> {
                        User updatedUser = CreateUpdateMapper.updateUser(user, hostCreateUpdateRequest.getUser(),
                                createUpdateDateTime);
                    ...
                    .onErrorResume(error -> Mono.error(new ResGraphException(error.getMessage(),
                            HttpStatus.BAD_REQUEST)))
                    .switchIfEmpty(Mono.error(new ResGraphException(
                            ResConstants.USER_NOT_FOUND_WITH_ID + hostCreateUpdateRequest.getUser().getUserId(),
                            HttpStatus.NOT_FOUND)));
        } else if (hostCreateUpdateRequest.getUser().getLastName() != null
                && hostCreateUpdateRequest.getUser().getPrimaryContactMethod() != null) {
            // todo: what if i want to update primary contact method? it'll create a new
            // user here...
            ...
            return fetchByPrimaryContactInfo(hostCreateUpdateRequest)
                    ...
                    .onErrorResume(
                            error -> Mono.error(new ResGraphException(error.getMessage(), HttpStatus.BAD_REQUEST)))
    ...
    private Mono<User> fetchByPrimaryContactInfo(HostCreateUpdateRequest hostCreateUpdateRequest) {
        if (hostCreateUpdateRequest.getUser().getPrimaryContactMethod().equals(ContactMethod.PHONE)) {
            return userRepository.findByLastNameAndPrimaryPhone(
                    ...
                    hostCreateUpdateRequest.getUser().getPhone().stream().filter(Phone::isPrimary).toList().get(0)
                            .getValue());
        } else {
            return userRepository.findByLastNameAndPrimaryEmail(
                    hostCreateUpdateRequest.getUser().getLastName(),
                    hostCreateUpdateRequest.getUser().getEmail().stream().filter(Email::isPrimary).toList().get(0)
                            .getValue());
        }
    }

    private Mono<HostCreateUpdateResponse> createNewHost(HostCreateUpdateRequest hostCreateUpdateRequest,
        ...
        return hostRepository.save(host)
                .map(newHost -> new HostCreateUpdateResponse(ResConstants.HOST_CREATE + host.getHostId(), newHost,
                        null));
    }
    ...
    private Mono<HostCreateUpdateResponse> zipUserUpdateWithHostUpdate(HostCreateUpdateRequest hostCreateUpdateRequest,
            Host hostToUpdate,
            Host updatedHost,
            String createUpdateDateTime) {
        ...
        // if there are no updates to apply to the user, an error is thrown and no
        // updates are applied
        return updateUserInfo(hostCreateUpdateRequest, hostToUpdate.getUserId(),
                ...
                .flatMap(user -> hostRepository.save(updatedHost)
                        ...
                        .flatMap(hostAndUserTuple -> Mono.just(new HostCreateUpdateResponse(
